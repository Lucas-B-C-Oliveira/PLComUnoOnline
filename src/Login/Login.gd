...
func _on_LoginButton_pressed() -> void:
    ...
    Firebase.login(username.text + "@godot.com", password.text, http)
...
func connect_signals_with(gm_ref, func_name: String = "") -> void:
    ...
    if gm_ref.has_method(func_name) and !self.is_connected("signal_login_button_pressed_in_Login", gm_ref, func_name):
